...
x_data = tf.placeholder(tf.int32, [setting.BATCH_SIZE, None])
y_data = tf.placeholder(tf.int32, [setting.BATCH_SIZE, None])

data = dataset.Dataset(setting.BATCH_SIZE)  # 创建数据集

model = TrainModel(x_data, y_data)  # 创建训练模型
...
with tf.Session() as sess:
    ...
    for step in range(TRAIN_TIMES):
        ...
        x, y = data.next_batch()
        # 计算loss
        loss, _ = sess.run([model.loss, model.optimize], {model.data: x, model.labels: y})
        if step % SHOW_STEP == 0:
